...
class TicketRevisionRevertForm extends ConfirmFormBase {
  ...
  public static function create(ContainerInterface $container) {
    ...
    $instance->ticketStorage = $container->get('entity_type.manager')->getStorage('event_ticket');
  ...
  public function getFormId() {
    return 'event_ticket_revision_revert_confirm';
  }
   ...
   * {@inheritdoc}
   */
  public function buildForm(array $form, FormStateInterface $form_state, $event_ticket_revision = NULL) {
    $this->revision = $this->TicketStorage->loadRevision($event_ticket_revision);
  ...
  public function submitForm(array &$form, FormStateInterface $form_state) {
    ...
    $this->messenger()->addMessage(t('Ticket %title has been reverted to the revision from %revision-date.', ['%title' => $this->revision->label(), '%revision-date' => $this->dateFormatter->format($original_revision_timestamp)]));
